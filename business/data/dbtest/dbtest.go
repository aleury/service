...
import (
	...
	"github.com/aleury/service/business/data/dbmigrate"
...
func NewTest(t *testing.T, c *docker.Container) *Test {
	...
	if err := database.StatusCheck(ctx, dbM); err != nil {
		t.Fatalf("status check dataabase: %v", err)
	}
	...
	t.Log("Migrate and seed the database")
	...
	// teardown is the function that should invoked when the caller is done
	// with the database.
	teardown := func() {
		t.Helper()
		db.Close()
...
// CoreAPIs represents all of the core api's needed for testing.
